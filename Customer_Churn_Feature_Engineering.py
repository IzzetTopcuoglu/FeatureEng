...
df = pd.read_csv("C:/Users/izzet/PycharmProjects/DSMLBC9/DSMLBC9- Ödevler/Modül_5_Özellik_Mühendisliği/Telco-Customer-Churn.csv")
df.head()
df.shape
df.info()
...
for col in cat_cols:
    df.groupby([col, 'Churn']).agg({"Churn":"count"})
...
df["Average_Charge"]=df["TotalCharges"]/df["tenure"]
df["new_increase"]=df["Average_Charge"]/df["MonthlyCharges"]
df.head()
df["gender"]=df["gender"].apply(lambda x: 1 if x=="Female" else 0)
df.loc[((df['gender']==0)&(df["SeniorCitizen"]==1)),'new_gender'] ="senior_male"
df.loc[((df['gender']==0)&(df["SeniorCitizen"]==0)),"new_gender"] ="young_male"
df.loc[((df['gender']==1)&(df["SeniorCitizen"]==1)),"new_gender"] ="senior_female"
df.loc[((df['gender']==1)&(df["SeniorCitizen"]==0)),"new_gender"] ="young_female"
df.groupby("new_gender").agg({"Churn":"count"})
